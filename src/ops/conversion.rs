//! Conversion op family — Wave-1 pre-declared **stub** (`CLI_CONTRACT.md` §6).
//!
//! Registered in [`super::FAMILIES`] now so the conversion wave fills in only this
...
/// The clap commands this family contributes (none until the conversion wave lands).
pub fn commands() -> Vec<Command> {
    Vec::new()
}

/// Static per-command metadata (none until the conversion wave lands).
...
/// # Errors
///
/// Always errors in Wave 1: the conversion family registers no commands yet, so
