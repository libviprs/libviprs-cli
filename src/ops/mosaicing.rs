//! Mosaicing op family — Wave-1 pre-declared **stub** (`CLI_CONTRACT.md` §6).
//!
//! Registered in [`super::FAMILIES`] now so the mosaicing wave fills in only this
...
/// The clap commands this family contributes (none until the mosaicing wave lands).
pub fn commands() -> Vec<Command> {
    Vec::new()
}

/// Static per-command metadata (none until the mosaicing wave lands).
...
/// # Errors
///
/// Always errors in Wave 1: the mosaicing family registers no commands yet, so
