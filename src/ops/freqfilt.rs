//! Frequency-filter op family — Wave-1 pre-declared **stub** (`CLI_CONTRACT.md` §6).
//!
//! Registered in [`super::FAMILIES`] now so the freqfilt wave fills in only this
...
/// The clap commands this family contributes (none until the freqfilt wave lands).
pub fn commands() -> Vec<Command> {
    Vec::new()
}

/// Static per-command metadata (none until the freqfilt wave lands).
...
/// # Errors
///
/// Always errors in Wave 1: the freqfilt family registers no commands yet, so
